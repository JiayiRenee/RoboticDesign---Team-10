...
from nav.robot_navigator import BasicNavigator, NavigationResult
...
class MainWindow(QMainWindow):
    ...
    def start_state_machine(self):
        if self.check_connection_nuc():
            subprocess.run(["ssh", "username@nuc_ip_address", "ros2 run nav robot_state_machine"])
        else:
            subprocess.run(["ros2", "run", "nav", "robot_state_machine"])
